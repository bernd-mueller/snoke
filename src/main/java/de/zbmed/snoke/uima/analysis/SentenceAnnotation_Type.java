...
/**
 * Custom SentenceAnnotation_Type for having a unique tag for sentences.
 *
...
public class SentenceAnnotation_Type extends Annotation_Type {
  ...
  private final FSGenerator fsGenerator = new FSGenerator() {
    public FeatureStructure createFS(int addr, CASImpl cas) {
      if (instanceOf_Type.useExistingInstance) {
        ...
        if (null == fs) {
          fs = new SentenceAnnotation(addr, instanceOf_Type);
          instanceOf_Type.jcas.putJfsFromCaddr(addr, fs);
          return fs;
        }
        return fs;
      } else
        return new SentenceAnnotation(addr, instanceOf_Type);
    }
  };
  ...
  protected SentenceAnnotation_Type() { // block default new operator
